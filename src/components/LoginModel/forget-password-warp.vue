  ...
  >
    <template #header="{ close }">
      ...
      <div class="my-header-right" @click="handleEmailRetrieval">
        <template v-if="isStateType === 1">
          ...
          <span class="dialog-title">{{ "通过邮箱找回" }}</span>
        </template>
        <template v-else>
          <el-icon color="var(--color_main)" size="20"><Message /></el-icon>
          <span class="dialog-title">{{ "通过手机找回" }}</span>
        </template>
      </div>
    </template>
    <el-form
    ...
    >
      ...
      <template v-else>
        ...
        <!-- 邮箱验证码 -->
        <el-form-item prop="emailCode" label="邮箱验证码" label-position="top">
          <el-input
      ...
      <el-form-item prop="password" label="新密码" label-position="top">
        <el-input
  ...
  try {
    ...
    const params = isPhone
      ? {
          ...
          verifyCodeType: "change_password",
        }
      : {
          email: mobileOrEmail,
...
const handeSubmitForget = async () => {
  try {
    ...
    let parms = {
      ...
      phoneOrEmail: isStateType.value === 1 ? formValue.phone : formValue.email,
      verifyCode: isStateType.value === 1 ?  formValue.phoneCode : formValue.emailCode,
